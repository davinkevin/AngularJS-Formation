...
gulp.task('js', () =>
    es.merge(
        gulp.src(htmlLocation).pipe(plumber()).pipe(templateCache('partial.js',{ standalone : true, module: "cf.partial" })).pipe(concat("partials.js")),
        gulp.src(angularAppLocation)
    )
...
gulp.task('watch',['js', 'less', 'inject', 'webserver'], () => {
    gulp.watch(indexLocation, () => gulp.src(indexLocation).pipe(connect.reload()) );
